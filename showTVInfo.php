...
<p></p>
            ...
            $cont = file_get_contents("http://www.imdb.com/find?q=UFO&s=tt&ref_=fn_al_tt_mr");
   $cont = htmlentities($cont, ENT_QUOTES);
$html1=$cont;
   $html1 = substr($html1,5000,66666);
  $cont=$html1;

   for ($i=0; $i<=strlen($cont) ;$i+=1)
  {
  if($cont[$i]=='t' && $cont[$i+1] =='i' && $cont[$i+2] =='t' && $cont[$i+3]=='l' && $cont[$i+4] =='e' && $cont[$i+5] =='/'  && $cont[$i+6] =='t')
{
   $toprint2 =substr($cont,$i+43,100);
$j=1;
if($toprint2[$j]!='l' && $toprint2[$j+1]!='t' && $toprint2[$j]!='&' && $toprint2[$j+1]!='l')
{
echo $toprint2;
echo '</br>';
}
